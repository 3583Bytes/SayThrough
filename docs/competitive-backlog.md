...
| **Printable companion boards (PDF)** | Backup when the device dies; free classroom copies (D-10). Trivial on web via print-to-PDF; genuinely unique. **Shipped:** Settings → Print outputs the user's OWN board (§14.4) — their buttons, symbols, photos and colours, in the real grid, hidden buttons leaving their cells empty so the motor plan transfers; plus `/printable-boards/` on the site for the default boards | M | [x] |
