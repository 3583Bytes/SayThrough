...
The app ships one authored page set **per (language, size)** pair. Adding a
language is not a translation pass over an existing board — the board itself
is authored again, because the grammar changes what belongs on it.

**Data.** `src/data/coreWords.json` (English) and `coreWords.es.json`
(Spanish, generated by `scripts/vocabulary/build-spanish-core.mjs` — edit the
generator, not the JSON). Same schema: one entry per grid size, each word
...
Spanish lives in `morphology.es.ts`. They are separate engines, not one
parameterised engine, for two reasons: Spanish offers PERSON forms of a verb,
and Spanish adjectives and determiners AGREE in gender and number with their
noun. Agreement means a form is only correct relative to another word, so
`wordForms()` takes a `MorphContext` carrying the message bar; the agreeing
form is offered first and hinted `concuerda`. Gender is inferred from the
word's ending with an exceptions table (`el día`, `la mano`, `el agua`).
...
| Prediction lexicon | `public/prediction/<lang>.txt`, built by `npm run lexicon`; per-language blocklist (`blocklist.<lang>.json`) and letter class |
...
| Enhanced voice | One Piper model + espeak voice per language; only the active language's ~60–75 MB model is fetched |
...
**§19.6 applies per language.** The Spanish boards are adapted from published
Spanish core vocabulary work by a developer, exactly as the English ones were,
and are **equally unreviewed**. The release gate is one SLP sign-off per
language, from someone qualified in that language's AAC practice — a Spanish
board reviewed by an English-speaking clinician is not a review.
