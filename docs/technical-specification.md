...
App Bundle (shipped with app binary, ~180 MB total)
  ...
  assets/symbolIndex/{lang}.json ← { id, library, keywords[] }, one per language
  assets/vocabulary/
...
App Bundle (served from static hosting)
  ...
  assets/symbolIndex/{lang}.json ← symbol keywords for search, one per language
  assets/vocabulary/
    core-vocabulary.obf
    quick-phrases.obf
...
- Search is local (offline) — searches symbolIndex/{lang}.json for the
  profile’s language, so a Polish board is customised with Polish words
...
scripts/
  ...
  6-buildSymbolIndex.js         ← generates assets/symbolIndex/{lang}.json
```

**symbolIndex/{lang}.json structure:**
```json
// symbolIndex/pl.json — the SAME pictogram ids in every language,
// only the keywords differ. The picker fetches one file: the profile's.
[
  {
    "id": "arasaac:2349",
    "library": "arasaac",
...
export async function searchSymbols(keyword: string): Promise<SymbolResult[]> {
  // the profile’s symbolIndex is loaded into memory on first search (~1MB)
...
| Symbol picker | Local search from symbolIndex/{lang}.json |
...
│   ├── symbolIndex/{lang}.json  ← Symbol keywords per language (generated)
...
- Fetched lazily on first keyboard open, then held in module memory; the
  ...
  entry, matching `symbolIndex/{lang}.json`)
...
- The core region is identical (labels + positions) across all pages of
  every authored size **in every language** — asserted per (language, size)
  in `tests/unit/vocabulary.test.ts`, alongside checks that no button lands
  off-grid or shares a cell, and that raising a level never moves a button
- A practicing SLP has reviewed the word selection and layout **for each
  language**, and the sign-off is documented in the repo. Outstanding for
  both English and Spanish (§19.7)


### 19.7 Languages

The app ships one authored page set **per (language, size)** pair — currently
English, Spanish, Polish and Brazilian Portuguese. Adding a language is not a translation pass over
an existing board: the board itself is authored again, because the grammar
changes what belongs on it.

**Data.** `src/data/coreWords.json` (English) plus `coreWords.{es,pl,pt}.json`
(generated by `scripts/vocabulary/build-{spanish,polish,portuguese}-core.mjs`
— edit the generator, not the JSON). Same schema: one entry per grid size, each word
`[label, partOfSpeech, level]`. Symbol maps pair with them
(`seedSymbolMap.json` / `seedSymbolMap.es.json`); ARASAAC ids are
language-neutral, so the Spanish map reuses the English concepts' ids rather
than re-running the symbol pipeline.

**Ids.** English keeps the unsuffixed ids it shipped with
(`builtin-core-vocabulary`, `builtin-core-home`, `builtin-core-<topic>`);
every other language is suffixed (`builtin-core-es-5x6-…`). Existing profiles,
word lists and user pages are therefore untouched by adding a language.

**What differs in Spanish, and why it is not a translation:**

- **Two copulas.** `ser` and `estar` are both high-frequency and not
  interchangeable, so both sit in the persistent core, and the *Palabras de
  apoyo* page carries their person forms. This is the Spanish counterpart of
  the English "to be" audit (§19.4) and a bigger hole, because there are two
  verbs to miss instead of one.
- **Pro-drop.** Person lives on the verb, so the constantly-used frame words
  (`quiero`, `necesito`, `gusta`, `tengo`) are carried already conjugated;
  everything else is an infinitive the word-forms popup conjugates.
- **`gustar` is not `like`.** It takes an experiencer clitic, so the core
  carries `gusta`, not the infinitive.
- **Determiners inflect**, so they earn core-page space the English board
  spends elsewhere.

**What differs in Polish:**

- **CASE, and it is governed rather than agreed.** A Polish noun changes shape
  according to the verb or preposition in front of it: `chcę wodę`
  (accusative) but `nie chcę wody` (genitive, because the clause is negated).
  Nothing upstream can predict that, so the board carries the mianownik and
  the word-forms popup *offers* the cases, labelled with the question that
  selects each one (`kogo? czego?`). This is the first thing neither the
  English nor the Spanish engine could do.
- **One copula, heavily irregular.** `być` has no predictable forms, so the
  *Słowa pomocnicze* page carries them explicitly.
- **No articles.** The cells Spanish spends on el/la/un/una go to prepositions,
  which Polish needs more of because they govern case.
- **`się` earns a core cell.** A large share of everyday verbs are reflexive
  and the particle is written separately.

**What differs in Brazilian Portuguese:**

- **CONTRACTIONS, which are not morphology.** `de` + `o` is `do`, `em` + `a` is
  `na`, `a` + `o` is `ao` — obligatorily. `eu vou a o parque` is not clumsy,
  it is ungrammatical, and a user tapping two buttons has no way to fix it. So
  `src/services/contractions.ts` fuses the two words **at append time**, in
  `messageStore`, not in the word-forms popup: nothing about the button
  labelled `de` changes until the word after it exists. Making the fused form a
  single token (rather than rendering it over two) keeps the message bar, the
  word-by-word highlight, delete-last-word and history all correct with no
  special cases — and matches the grammar, since `do` really is one word.
  `de` and `em` therefore earn persistent-core cells that Spanish spends
  elsewhere.
- **NASAL AND CONSONANT PLURALS.** `-ão` splits three ways with no predictive
  rule (pão→pães, mão→mãos, coração→corações), and `-l`→`-is`, `-m`→`-ns` have
  no Spanish counterpart.
- **FEWER PERSON FORMS.** Brazilian usage puts `você` on the third person, so
  the paradigm is eu / você-ele / nós / vocês-eles — four forms, not Spanish's
  five.
- **pt-BR, not pt-PT**, throughout: strings, vocabulary and voice.

**Word forms.** `morphology.ts` dispatches by language; English stays there,
Spanish in `morphology.es.ts`, Polish in `morphology.pl.ts`, Portuguese in
`morphology.pt.ts`. They are separate engines, not one parameterised engine,
because each needs machinery the others do not: Spanish offers PERSON forms and
agrees adjectives in gender/number; Polish adds case, the masculine-personal
plural, and aspect pairs; Portuguese adds nasal plurals and needs a
sentence-level contraction pass that is not morphology at all. Agreement
means a form is only correct relative to another word, so `wordForms()` takes
a `MorphContext` carrying the message bar; the agreeing form is offered first
and hinted (`concuerda` / `zgadza się`). Gender is inferred from the word's
ending with a per-language exceptions table (`el día`, `la mano`; `tata`,
`noc`).

**Grammatical gender of the SPEAKER (`UserProfile.grammaticalGender`).** Polish
marks the speaker's own gender in the past tense, in the first and second
person: `byłem` vs `byłam`. A board offering only the masculine would misgender
a female user every time she talked about yesterday — in her own voice. The
profile therefore carries an optional grammatical gender, surfaced in Settings
**only for languages that mark it**, and the matching form leads. Unset means
both are offered and labelled; the other form is never removed, so the user can
always override. Polish Quick Phrases avoid the past tense entirely for the
same reason — one fixed set cannot be right for every speaker.

**Everything else that follows the profile's language:**

| Subsystem | Mechanism |
|---|---|
| UI strings | `src/i18n/` — `en.ts` is canonical, other tables are typed `Record<StringKey, string>` so a missing translation fails the build. `useT()` binds to the active profile |
| Prediction lexicon | `public/prediction/<lang>.txt`, built by `npm run lexicon`; per-language blocklist (`blocklist.<lang>.json`) and letter class — Polish needs `ąćęłńóśźż` or `się` and `już` vanish |
| Next-word frames | `seedBigrams.json`, keyed by language code |
| Symbol search | `public/symbolIndex/<lang>.json`, built by `build-symbol-index.mjs` from a per-locale ARASAAC index. The pictograms are language-neutral **images**; their keywords are not, and searching English only meant a Polish board had to be customised by guessing English words. Only the profile's ~1 MB index is fetched. ARASAAC's locale for Brazilian Portuguese is `br`, not `pt`. Matching folds case and accents (and Polish `ł`), so `agua` finds `água` and `jesc` finds `jeść`. **Upstream coverage decays:** ARASAAC's Polish effort stopped in 2024, so `keyword-overrides.pl.json` supplies Polish for the 1,090 pictograms it left untranslated — applied ONLY where upstream has none, so each entry retires itself when the real translation lands. Clock faces there are rule-generated (`gen-clock-keywords.mjs`) because Polish half-past names the *next* hour. Anything still uncovered borrows English rather than vanishing |
| Platform voice | `rankVoices(voices, language)` filters by BCP-47 and scores against a per-language known-good list |
| Enhanced voice | One Piper model + espeak voice per language (`en_US-hfc_female`, `es_ES-sharvard`, `pl_PL-gosia`, `pt_BR-faber`); only the active language's ~60–75 MB model is fetched |
| Contractions | `contractions.ts`, applied in `messageStore.appendToken`; a language with no table is untouched |
| Quick Phrases | Its own authored set per language, addressed via `quickPhrasesMetaKey(lang)` |

**Switching language** moves the profile to the **same grid size** in the new
language (`sameSizeInLanguage`) — never a different size, which would cost a
6×10 user two thirds of their vocabulary and every motor plan built on it —
and resets the voice selection and TTS engine, since neither carries over.
Pages, word lists, history and tracking are kept.

**§19.6 applies per language.** Every board is adapted from published core
vocabulary work by a developer, and every one is **equally unreviewed**. The
release gate is one SLP sign-off per language, from someone qualified in that
language's AAC practice — a Spanish board reviewed by an English-speaking
clinician is not a review. `npm run review-packet` generates one packet per
language, each written in the language it asks about.

**Guarding the morphology.** `tests/unit/morphologyPl.test.ts` runs *every word
the Polish boards ship* through the engine and fails on output shapes that mean
a rule fell through rather than fired (`kolegaa`, `butya`, `babciie`). Hand-
written examples missed all three; the sweep caught nine distinct rule failures
on its first run. `morphologyPt.test.ts` carries the same sweep, and it is now
the standard for any language with non-trivial morphology.
