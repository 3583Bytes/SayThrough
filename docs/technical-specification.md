...
English, Spanish and Polish. Adding a language is not a translation pass over
an existing board: the board itself is authored again, because the grammar
changes what belongs on it.

**Data.** `src/data/coreWords.json` (English), `coreWords.es.json` and
`coreWords.pl.json` (generated by `scripts/vocabulary/build-{spanish,polish}-core.mjs`
...
Spanish in `morphology.es.ts`, Polish in `morphology.pl.ts`. They are separate
engines, not one parameterised engine, because each needs machinery the others
do not: Spanish offers PERSON forms and agrees adjectives in gender/number;
Polish adds case, the masculine-personal plural, and aspect pairs. Agreement
...
| Enhanced voice | One Piper model + espeak voice per language (`en_US-hfc_female`, `es_ES-sharvard`, `pl_PL-gosia`); only the active language's ~60–75 MB model is fetched |
...
on its first run. Any language with non-trivial morphology should get the same
guard.
