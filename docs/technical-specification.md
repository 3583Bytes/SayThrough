...
| Symbol search | `public/symbolIndex/<lang>.json`, built by `build-symbol-index.mjs` from a per-locale ARASAAC index. The pictograms are language-neutral **images**; their keywords are not, and searching English only meant a Polish board had to be customised by guessing English words. Only the profile's ~1 MB index is fetched. ARASAAC's locale for Brazilian Portuguese is `br`, not `pt`. Matching folds case and accents (and Polish `ł`), so `agua` finds `água` and `jesc` finds `jeść`. Coverage is not complete in every language — Polish sits ~92% — so a pictogram with no keyword in the active language borrows the English one rather than becoming unreachable |
