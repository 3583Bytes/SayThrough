...
- **Multilingual**: English, German, Spanish, Italian, Polish, Brazilian Portuguese. One authored page set per (language, size) — a board is never machine-translated or reflowed. UI strings in `src/i18n` (`en.ts` canonical; other tables are typed so a missing key fails the build). Prediction lexicon, next-word frames, voice ranking and the Piper model all follow the profile's language. Each language has its own morphology engine (`morphology.{de,es,it,pl,pt}.ts`) — Spanish needs gender agreement, Polish needs case and the speaker's own grammatical gender, Portuguese and Italian need obligatory word-fusing contractions (`contractions.ts`, applied in `messageStore`; Italian fuses five prepositions with every article form). Italian keeps all six persons, because unlike Brazilian Portuguese it has not collapsed `voi`. German is the one engine that is not a port: it puts case on the **article** rather than the noun, and its separable verbs (`aufstehen` → *ich stehe … auf*) cannot be produced a word at a time, so the board leans on **modal + infinitive** (`ich will aufstehen`) and the conjugated form keeps its particle attached as one token. German gender is not predictable, so every board noun is tabled and a unit test fails if one is missing — 108 of 215 were silently defaulting to masculine when German first landed. See technical-specification.md §19.7
...
npm run lexicon    # rebuild prediction lexicons (en/de/es/it/pl/pt) from OpenSubtitles
npm run spanish-core  # regenerate the Spanish boards + symbol map
npm run german-core   # regenerate the German boards + symbol map
npm run italian-core  # regenerate the Italian boards + symbol map
...
npm run voice      # fetch the English Piper model (voice:de/es/it/pl/pt too)
```
...
The marketing site is **generated, not hand-authored**: `site/_templates/` holds the page structure and `site/_content/{en,de,es,it,pl,pt}.json` the copy, rendered by `scripts/build-site.mjs` into one real HTML file per (language, page) — 16 pages × 6 languages under `/`, `/de/`, `/es/`, `/it/`, `/pl/`, `/pt/`, plus an English-only 404 and one market-scoped page. A `languages` field on a `PAGES` entry narrows it to the markets it is about (the MÓWik comparison is Polish-only); such a page emits no hreflang alternates, its switcher sends other languages to their home page, and its copy keys live only in those language files. `en.json` is canonical and the build fails on a key any language is missing. Each page carries its own canonical + reciprocal `hreflang` (`x-default` → English; Portuguese declares both `pt-BR` and a bare `pt`), and the sitemap lists all of them with a `lastmod` taken from git — never `changefreq`/`priority`, which Google ignores.

Every page emits `Organization` + `WebSite` JSON-LD; breadcrumbed pages emit `BreadcrumbList` from the same `crumbs` array the template renders visibly, and guides/comparisons emit `Article` with git-derived dates. `/guides/core-words/` publishes the real vocabulary, **generated from `src/data/coreWords.*.json`** with the Fitzgerald colours parsed out of `src/constants/colors.ts`, so the page can't drift from the product. Screenshots live in `site/img/` (WebP + a PNG fallback, light and dark), regenerated from `docs/screenshots/` — see §5 of `docs/competitive-backlog.md` for the search/referral backlog and what's still outstanding off-page.
